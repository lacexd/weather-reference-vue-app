<template>
<div v-if="isAppLoading" class="loading-container">
  <div class="preloader">
    <svg class="svg-container" version="1.1" id="sun" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="10px" height="10px" viewBox="0 0 10 10" enable-background="new 0 0 10 10" xml:space="preserve"
      style="opacity: 1; margin-left: 0px; margin-top: 0px;">
      <g>
        <path class="path-container" fill="red" stroke="yellow"
        ...
        <path class="path-container" fill="red" stroke="yellow" d="M2.202,5.146c0.082,0,0.149-0.065,0.149-0.147S2.284,4.851,2.202,4.851H1.503c-0.082,0-0.148,0.066-0.148,0.148s0.066,0.147,0.148,0.147H2.202z"></path>
        <path class="path-container" fill="red" stroke="yellow" d="M8.493,4.851H7.794c-0.082,0-0.148,0.066-0.148,0.148s0.066,0.147,0.148,0.147l0,0h0.699c0.082,0,0.148-0.065,0.148-0.147S8.575,4.851,8.493,4.851L8.493,4.851z"></path>
        <path class="path-container" fill="red" stroke="yellow" d="M5.146,2.203V0.805c0-0.082-0.066-0.148-0.148-0.148c-0.082,0-0.148,0.066-0.148,0.148v1.398c0,0.082,0.066,0.149,0.148,0.149C5.08,2.352,5.146,2.285,5.146,2.203z"></path>
        <path class="path-container" fill="red" stroke="yellow" d="M4.85,7.796v1.396c0,0.082,0.066,0.15,0.148,0.15c0.082,0,0.148-0.068,0.148-0.15V7.796c0-0.082-0.066-0.148-0.148-0.148C4.917,7.647,4.85,7.714,4.85,7.796z"></path>
        <path class="path-container" fill="red" stroke="yellow"
    ...
    <svg class="svg-container" version="1.1" id="cloud" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="10px" height="10px" viewBox="0 0 10 10" enable-background="new 0 0 10 10"
      xml:space="preserve">
      <path class="path-container" fill="none"
...
export default {
  ...
  data() {
    return {
      loadingTexts: ['Opening the window', "Checking the weather", "Just a second and we are there"],
      loadingText: 'Opening the window',
  ...
  mounted() {
    const intervalId = setInterval(function() {
      let textInd = 0
      this.loadingTexts.find((text, ind) => {
        textInd = ind
        return this.loadingText === text
      })
      if (textInd === this.loadingTexts.length - 1) {
        this.isAnimationOver = true
        clearInterval(intervalId)
      } else {
        this.loadingText = this.loadingTexts[textInd + 1]
      }
    }.bind(this), 3000)
  },
  computed: {
    ...
    isAppLoading() {
      return !this.isAnimationOver && !this.isAppLoaded
    }
  }
}
</script>
